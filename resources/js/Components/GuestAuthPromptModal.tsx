...
export default function GuestAuthPromptModal({ isOpen, onClose }: GuestAuthPromptModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-black/75 backdrop-blur-md flex items-center justify-center p-4 animate-fadeIn">
            <div
                className="relative w-full max-w-md bg-white dark:bg-slate-900 border border-slate-300/80 dark:border-slate-700/80 rounded-3xl p-6 shadow-2xl overflow-hidden"
            ...
            >
                ...
                <div className="flex items-center gap-3">
                    <button
                        type="button"
                        onClick={onClose}
                        className="w-1/3 py-2.5 px-4 rounded-xl border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:text-white hover:bg-slate-100 dark:bg-slate-800 text-sm font-semibold transition-colors text-center"
                    >
                        Batal
                    </button>
                    <Link
